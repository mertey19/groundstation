...
namespace Mapbox.Platform
{
	...
	public class Response
	{
		...
		public static Response FromWebResponse(IAsyncRequest request, UnityWebRequest apiResponse, Exception apiEx)
		{
			...
			// on UWP connection errors may be reported unexpectedly
			bool hasConnectionError;
#if UNITY_2020_2_OR_NEWER
			hasConnectionError = apiResponse.result == UnityWebRequest.Result.ConnectionError;
#else
			hasConnectionError = apiResponse.isNetworkError;
#endif
			if (hasConnectionError && !string.IsNullOrEmpty(apiResponse.error))
			{
