...
namespace Mapbox.Unity.Utilities
{
	...
	public static class Conversions
	{
		...
		/// Expects format: latitude, longitude (e.g. "37.7749,-122.4194")
		/// </summary>
		...
		public static Vector2d StringToLatLon(string s)
		{
			...
			{
				Debug.LogWarning("[Mapbox.Conversions] StringToLatLon: null or empty string, returning (0,0). Check Map options default location.");
				return new Vector2d(0, 0);
			}

			// Normalize: European decimal comma (37,78 -> 37.78), remove spaces (e.g. "37,78, -122,40" -> "37.78,-122.40")
			s = s.Trim();
			...
			{
				Debug.LogWarning("[Mapbox.Conversions] StringToLatLon: expected \"lat,lon\" format, got \"" + s + "\". Returning (0,0).");
				return new Vector2d(0, 0);
			}

			double latitude = 0;
			double longitude = 0;
			string latStr = latLonSplit[0].Trim();
			string lonStr = latLonSplit[1].Trim();

			if (!double.TryParse(latStr, NumberStyles.Any, NumberFormatInfo.InvariantInfo, out latitude))
			{
				Debug.LogWarning("[Mapbox.Conversions] StringToLatLon: could not parse latitude \"" + latStr + "\". Returning (0,0).");
				return new Vector2d(0, 0);
			}

			if (!double.TryParse(lonStr, NumberStyles.Any, NumberFormatInfo.InvariantInfo, out longitude))
			{
				Debug.LogWarning("[Mapbox.Conversions] StringToLatLon: could not parse longitude \"" + lonStr + "\". Returning (0,0).");
				return new Vector2d(0, 0);
			}
